...
function renderBooks(books) {
  ...
  if (!books.length) {
    hideEl(booksTable);
    showEl(booksEmpty);
    if (totalBooks) totalBooks.textContent = "0";
    return;
  }
    ...
    return `
      <tr>
        ...
        <td class="text-center">
          <button class="btn btn-sm btn-primary action-btn edit-trigger-btn me-1"
            title="Editar"
            data-id="${book.id}"
            ...
            data-total="${book.totalCopies || 0}"
            data-available="${available}">
            <i class="bi bi-pencil"></i>
          </button>
          <button class="btn btn-sm btn-danger action-btn delete-trigger-btn"
            title="Eliminar"
            data-id="${book.id}"
            data-title="${book.title}">
  ...
  // Asignar eventos dinámicamente para evitar problemas de ámbito con onclick en módulos
  document.querySelectorAll(".edit-trigger-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      editBookId.value       = btn.dataset.id;
      editTitle.value        = btn.dataset.title;
      editAuthor.value       = btn.dataset.author;
      editIsbn.value         = btn.dataset.isbn;
      editCategory.value     = btn.dataset.category;
      editTotalCopies.value  = btn.dataset.total;
      editAvailable.value    = btn.dataset.available;
      editModal.show();
    });
  });
  ...
  document.querySelectorAll(".delete-trigger-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      deleteBookId.value = btn.dataset.id;
      deleteBookTitle.textContent = btn.dataset.title;
      delModal.show();
    });
  });
}
